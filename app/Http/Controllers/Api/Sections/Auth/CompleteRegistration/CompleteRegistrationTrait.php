            ...
            if ((int) $request->account_type === User::ACCOUNT_TYPE_REAL_ESTATE) {
                ...
                if ($request->has('office_logo')) {
                    try {
                        ...
                        if ($imageData === false) {
                            throw new Exception('Invalid office logo data.');
                        }
                        $image = Image::make($imageData)->orientate();
                        $imageName = 'office_logo_' . $user->id . '_' . time() . '.webp';
                        ...
                        $image->encode('webp', 85)->save(public_path($imagePath));
                ...
                if ($request->has('business_logo')) {
                    try {
                        ...
                        if ($imageData === false) {
                            throw new Exception('Invalid business logo data.');
                        }
                        $image = Image::make($imageData)->orientate();
                        $imageName = 'business_logo_' . $user->id . '_' . time() . '.webp';
